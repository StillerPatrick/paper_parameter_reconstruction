...
import numpy as np
import os
from DataSetSigmaFsize import ScatterImageSet
...
class FC:
    ...
    def build(self,learning_rate):
        ...
        self.y = tf.placeholder(tf.float64,[None,2],name="y")
        ...
        fc1 = tf.layers.dense(inputs=features,units=2048,activation=tf.nn.relu,name="FC1")
        ...
        fc4 = tf.layers.dense(inputs=fc3,units=64,activation=tf.nn.relu,name="FC4")
        self.predict_op = tf.layers.dense(inputs=fc4, units=2, activation=None, name="Output")
    ...
    def fit(self, epochs, batch_size, dataset,model):
        ...
        for epoch in range(epochs):
            ...
            for idx in batch_idx:
                ...
                dirac = create_dirac(pitches)
                # current batch over the training
                _, c, prediction = sess.run([self.train_op, self.loss,self.predict_op], feed_dict={self.x: epoch_x, self.y: epoch_y,self.dirac: dirac})
            ...
            if (epoch + 1) % 2 == 0:
                ...
                dirac= create_dirac(pitches)
                summary, c = sess.run([merged, self.loss], feed_dict={self.x: dataset.validation_x,self.dirac:dirac, self.y: dataset.validation_y})
                ...
                pitches = pitch_model.predict(training_x)
                dirac = create_dirac(pitches)
                summary, c, prediction = sess.run([merged, self.loss,self.predict_op], feed_dict={self.x: training_x,self.y: training_y,self.dirac: dirac})
        ...
        for idx in range(num_test_batches):
            ...
            pitches = pitch_model.predict(test_x)
            diracs = create_dirac(pitches)
            dir_path = os.path.dirname(os.path.realpath(__file__))
            print(dir_path)
            #Adding for Dataanalysis
            self.saver.restore(sess,self.save_dir+'-'+str(best_epoch))
            #self.saver.restore(sess,self.save_dir)
            prediction = sess.run(self.predict_op,feed_dict={self.x:test_x,self.dirac:diracs})
            prediction = np.reshape(prediction,(batch_size,2))
            predictions.extend(prediction)
            for i in range(batch_size):
                print("Label:",test_y[i],"Prediction:",prediction[i])
        np.save('test_training_fc1_predictions.npy',predictions)
        np.save('test_training_fc1_labels.npy',labels)
        sess.close()
